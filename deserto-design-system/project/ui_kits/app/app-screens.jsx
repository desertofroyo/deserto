...
(function () {
  ...
  const { Icon, Photo, StatusBar, TabBar, ISO, ISO_W } = window.AppKit;
    ...
    return (
      <div style={{ flex: 1, display: "flex", flexDirection: "column", background: "var(--wine-700)", color: "var(--cream-50)" }}>
        ...
        <div style={{ padding: "0 24px 40px", display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <NS.Button variant="accent" size="lg" fullWidth onClick={onContinue}>Continue</NS.Button>
  ...
  /* ---------- Home ---------- */
  function Home({ onBuild, onOpenProduct }) {
    const { user, categories, featured } = window.APP;
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
        ...
        <div style={{ padding: "4px 22px 20px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <NS.Avatar name={user.name} tone="rose" size={46} />
          </div>
          ...
          <div style={{ marginTop: 16, background: "var(--wine-700)", borderRadius: "var(--radius-xl)", padding: 18, color: "var(--cream-50)", position: "relative", overflow: "hidden" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <NS.Badge tone="orange" variant="solid">{user.points} pts</NS.Badge>
            </div>
              ...
              <div key={p.id} onClick={() => onOpenProduct(p)} style={{ display: "flex", gap: 14, background: "var(--white)", border: "1px solid var(--border-default)", borderRadius: "var(--radius-lg)", padding: 12, cursor: "pointer", boxShadow: "var(--shadow-xs)" }}>
                ...
                <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center" }}>
                  <div style={{ display: "flex", gap: 6, marginBottom: 4 }}>
                    <NS.Badge tone={p.tag === "Vegan" ? "olive" : p.tag === "Popular" ? "orange" : "wine"}>{p.tag}</NS.Badge>
                    <NS.Badge tone="neutral">{p.cal} cal</NS.Badge>
                  </div>
  ...
  /* ---------- Build your bowl ---------- */
  function Build({ product, onBack, onAdd }) {
    const { bases, toppings, sizes } = window.APP;
    ...
    return (
      <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ padding: "20px 22px 12px" }}>
            ...
            <Section title="Choose your base" accent="var(--wine-700)">
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {bases.map((b) => (<NS.Tag key={b.key} selected={base === b.key} dotColor={b.tint} onClick={() => setBase(b.key)}>{b.key}</NS.Tag>))}
              </div>
            </Section>

            <Section title={"Toppings · " + (tops.length ? tops.length + " added" : "$0.75 each")} accent="var(--orange-500)">
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {toppings.map((t) => (<NS.Tag key={t} selected={tops.includes(t)} onClick={() => toggleTop(t)}>{t}</NS.Tag>))}
              </div>
            </Section>

            <Section title="Size" accent="var(--olive-600)">
              <div style={{ display: "flex", gap: 8 }}>
                {sizes.map((s) => (<NS.Tag key={s.key} selected={size === s.key} onClick={() => setSize(s.key)}>{s.key}{s.add ? " +" + money(s.add) : ""}</NS.Tag>))}
              </div>
            </Section>
          </div>
        </div>
        <div style={{ flex: "none", padding: "14px 22px 22px", borderTop: "1px solid var(--border-default)", background: "var(--white)", display: "flex", gap: 14, alignItems: "center" }}>
          ...
          <NS.Button variant="accent" size="lg" style={{ flex: 1 }} onClick={() => onAdd({ name: product ? product.name : (base ? base + " froyo" : "Custom froyo"), price: total, img: product ? product.img : "macro-swirl.jpg", pos: product ? product.pos : "center" })}>Add to bag</NS.Button>
        </div>
      </div>
    );
  }
  ...
  /* ---------- Bag ---------- */
  function Bag({ items, onRemove, onCheckout, onBrowse }) {
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
        ...
        {items.length === 0 ? (
          <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", color: "var(--text-muted)", padding: 30, textAlign: "center" }}>
            ...
            <NS.Button variant="primary" onClick={onBrowse}>Browse the menu</NS.Button>
          </div>
        ) : (
          <React.Fragment>
            <div style={{ flex: 1, overflowY: "auto", padding: "8px 22px" }}>
              ...
              <div style={{ marginTop: 16, background: "var(--leaf-200)", borderRadius: "var(--radius-lg)", padding: 14, display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div style={{ flex: 1, fontSize: 14, color: "var(--olive-700)", fontWeight: 600 }}>Pickup · Congress St</div>
            ...
            <div style={{ flex: "none", padding: "16px 22px 22px", borderTop: "1px solid var(--border-default)", background: "var(--white)" }}>
              ...
              <NS.Button variant="accent" size="lg" fullWidth style={{ marginTop: 12 }} onClick={onCheckout}>Place order · {money(total)}</NS.Button>
            </div>
          </React.Fragment>
        )}
      </div>
    ...
    return (
      <div style={{ flex: 1, display: "flex", flexDirection: "column", background: "var(--wine-700)", color: "var(--cream-50)" }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "0 34px", textAlign: "center" }}>
          <span style={{ width: 96, height: 96, borderRadius: "50%", background: "var(--orange-500)", display: "flex", alignItems: "center", justifyContent: "center", animation: "popIn .4s var(--ease-bounce)" }}><Icon name="check" size={52} color="var(--wine-900)" strokeWidth={3} /></span>
          <h2 style={{ color: "var(--cream-50)", fontSize: 30, margin: "24px 0 8px" }}>Order placed!</h2>
          <p style={{ fontFamily: "var(--font-editorial)", color: "var(--wine-200)", fontSize: 16 }}>We're on it. Your order will be ready for pickup at <strong style={{ color: "var(--cream-50)" }}>Congress St ~4:25pm</strong>.</p>
        ...
        <div style={{ padding: "0 24px 40px" }}>
          <NS.Button variant="accent" size="lg" fullWidth onClick={onDone}>Back to home</NS.Button>
        </div>
      </div>
    );
  }

  window.AppKit = Object.assign(window.AppKit || {}, { Welcome, Home, Build, Bag, Rewards, Confirm });
})();
